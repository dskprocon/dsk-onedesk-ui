...
import { getAllMembers, assignSiteWithHistory, updateMemberFields, updateSiteAssignmentHistory } from "../../firebase/services/registrationService";
...
import { MapPin, ClipboardList, Filter } from "lucide-react";
...
function BulkAssignSiteTeam({ name, role }) {
    ...
    // Load members and sites
    const loadMembers = async () => {
        const all = await getAllMembers();
        const headOfficeMembers = all.filter(m => m.category === "Head Office");
        setMembers(headOfficeMembers);
        setFiltered(headOfficeMembers);
    ...
    const handleAssign = async () => {
        if (assignSites.length !== 1) {
            showError("❌ Please select exactly 1 site to assign.");
            return;
        }
        ...
        try {
            ...
            if (success > 0) showSuccess(`✅ ${success} assigned successfully`);
            if (failed > 0) showError(`❌ ${failed} failed`);

            await loadMembers(); // refresh after assignment
        } catch (err) {
    ...
    const handleUnassign = async (memberId) => {
        const confirmed = await showConfirm("Unassign Member", "Do you want to unassign this member from all sites?");
        if (!confirmed) return;

        try {
            await updateMemberFields(memberId, { sites: [], teams: [] });

            const today = new Date();
            const toDate = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;

            await updateSiteAssignmentHistory(memberId, toDate);

            showSuccess("✅ Site unassigned successfully");
            await loadMembers(); 
        } catch (err) {
            showError("❌ Failed to unassign site");
        }
    };

    const headers = ["Select", "Name", "Category", "Role", "Sites", "Teams", "Actions"];
        ...
        <UniversalLayout name={name} role={role}>
            <div className="max-w-7xl mx-auto px-4 pt-6">
                <h2 className="text-2xl font-bold text-center mb-6 flex items-center gap-2">
                    <ClipboardList size={22} /> Bulk Assign Site / Team
                </h2>
                ...
                <button
                    ...
                    className="mb-8 flex items-center gap-2 bg-[#2F2F2F] hover:bg-[#505050] text-white font-semibold px-6 py-2 rounded shadow"
                >
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-300 space-y-6">
                    ...
                    <div className="flex justify-end pt-4">
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-2 rounded-xl font-semibold shadow ${
                                selectedIds.length === 0
                                    ? "bg-gray-300 text-gray-600 cursor-not-allowed"
                                    : "bg-green-600 hover:bg-green-700 text-white"
                            }`}
                        >
                            <MapPin size={16} /> Assign to {selectedIds.length} member(s)
                        </button>
                    </div>
                </div>
            </div>
